...
function Stream (db, id, cb, opts) {
  ...
  if (opts && opts.tracedebug) this.tracedebug = true

  if (!this.db.opened) throw new Error('not ready')

  var feedKey = util.streamIDToFeedKey(id)
  if (this.tracedebug) console.log('stream feed key retreived', feedKey)

  var self = this
  this.getMetadata(function (err, metadata) {
    if (self.tracedebug) console.log('got stream metadata', err)
  ...
  function onfeeds (err, dbfeed, contentfeed) {
    if (self.tracedebug) console.log('got stream feeds', err)
    if (err) return cb(err)
    self._dbfeed = dbfeed
    self.feed = contentfeed
    if (self._dbfeed.writable && self._dbfeed.length === 0) {
      self.db.authorized(feedKey, onauthorized)
    } else {
      done()
    }
  }

  function onauthorized (err, authorized) {
    if (self.tracedebug) console.log('onauthorized', err, authorized)
    if (err) return cb(err)
    if (authorized && self._dbfeed.writable && self._dbfeed.length === 0) {
      self._writeCheckpoint(done)
    } else {
      done()
    }
  }
  ...
  function done (err) {
    if (self.tracedebug) console.log('stream new done', err)
    if (err) return cb(err)
    cb(null, self)
  }
}
...
Stream.prototype.verify = function (checkpoint, cb) {
  if (this.tracedebug) console.log('verify checkpoint')
  ...
  this.feed.seek(checkpoint.byteLength ? checkpoint.byteLength - 1 : 0, { hash: true }, seeked)
...
Stream.prototype.findValidCheckpoint = function (opts, cb, invalidcb) {
  if (this.tracedebug) console.log('findValidCheckpoint')
  var self = this
  ...
  function seekValid (err, checkpoint) {
    if (self.tracedebug) console.log('seekValid', err)
    if (err) {
    ...
    self.verify(checkpoint, function (err) {
      if (self.tracedebug) console.log('verify done', err)
      if (err) {
...
// Write checkpoint publication reference
Stream.prototype.writeCheckPub = function (type, data, cb) {
  var path = 'checkpub/' + type
  this.writeAncillary(path, data, cb)
}
...
Stream.prototype._writeCheckpoint = function (cb) {
  if (this.tracedebug) console.log('_writeCheckpoint')
  var self = this
